...
const cleanState = async () => {
	await Promise.all([db.execute(sql`TRUNCATE telemetry_readings RESTART IDENTITY`), redis.flushdb()]);
};

const closeConnections = async () => {
	await Promise.all([redis.quit(), db.$client.end(), queue.close()]);
};

const whereRouteOrderByScenarioAsc = (route: routesType, metrics: PerformanceMetrics[]): PerformanceMetrics[] => {
	return metrics
		.filter((rounds) => rounds.route === route)
		.sort((a, b) => Number(a.scenario.split("C")[1] ?? 0) - Number(b.scenario.split("C")[1] ?? 0));
};
...
(async () => {
	...
	for (const { route, scenario } of rounds) {
		...
		if (route === "async") {
			while (
				await queue.getJobCounts().then((counts) => counts.waiting + counts.active + counts.delayed + counts.paused > 0)
			) {
      ...
      WHERE bucket > 1;`);
		...
		const drainResult = await db.execute<SpanLatency>(sql`
      SELECT EXTRACT(EPOCH FROM (MAX(created_at) - MIN(received_at))) AS span_s ,COUNT(*) as count
      FROM telemetry_readings
    `);

		const { count, span_s } = drainResult.rows[0];
		...
		const queryResults: InternalMetrics = {
			...
			drain_rps: span_s > 0 ? Number(count) / span_s : 0,
		};
	...
	for (const results of roundsResults) {
		console.log(`Cenário ${results.scenario} e rota ${results.route}`);
	}
	...
	const syncTextResult = syncSaturatedIteration
		? `A rota sync satura em ${syncSaturatedIteration.scenario} (${dropRatePercentage(syncSaturatedIteration)}% de drop)`
		: "A rota sync não saturou";
	const asyncTextResult = asyncSaturatedIteration
		? `A rota async satura em ${asyncSaturatedIteration.scenario} (${dropRatePercentage(asyncSaturatedIteration)}% de drop)`
		: "A rota async não saturou";
